...
void *get_container_ptr_by_member3(struct rcu_head *member3)
{
	...
	container_ptr -= offsetof(struct container, member3);
	return container_ptr;
}
...
void CONTAINER_free_callback(struct rcu_head *member3)
{
	...
	/* Getting the container structure from its member3 (from the "member3" inside "struct container") */
	container_ptr = get_container_ptr_by_member3(member3);
	free(container_ptr);
}
